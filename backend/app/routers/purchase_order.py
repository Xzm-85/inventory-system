...
@router.post("", response_model=PurchaseOrder, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_permission("purchase", "create"))])
...
@router.get("", response_model=list[PurchaseOrder], dependencies=[Depends(require_permission("purchase", "view"))])
def list_orders(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return get_purchase_orders(db, skip=skip, limit=limit)


# GET /purchase-orders/{order_id} —— 订单详情
@router.get("/{order_id}", response_model=PurchaseOrder, dependencies=[Depends(require_permission("purchase", "view"))])
